...
export class ZeroMd extends HTMLElement {
  ...
  constructor(defaults) {
    ...
    if (!this.constructor.ready) {
      this.constructor.ready = Promise.all([
        !!marked || this.loadScript(this.config.markedUrl),
  ...
  buildStyles() {
    ...
    const html = `<style>${this.config.hostCss}</style>${get('template[data-merge="prepend"]')}${get('template:not([data-merge])') ||
  ...
  async buildMd(opts = {}) {
    const src = async () => {
      this.base = window.baseUrl
      let fetchURI = new URL(this.src, this.base).href
      console.log(fetchURI, 1)
